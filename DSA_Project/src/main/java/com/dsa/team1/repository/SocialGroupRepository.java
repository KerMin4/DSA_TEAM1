...
public interface SocialGroupRepository extends JpaRepository<SocialGroupEntity, Integer> {
 ...
 // 필터링(검색어, 카테고리, 지역) + 정렬(조회수, 북마크, 이벤트날짜) 처리하는 쿼리
    @Query("SELECT g FROM SocialGroupEntity g " +
           ...
           "ORDER BY " +
           "CASE WHEN :sort = 'mostViewed' THEN g.viewCount END DESC, " +
           "CASE WHEN :sort = 'mostBookmarked' THEN g.bookmarkCount END DESC, " +
           "CASE WHEN :sort = 'upcomingEvents' THEN g.eventDate END ASC")
