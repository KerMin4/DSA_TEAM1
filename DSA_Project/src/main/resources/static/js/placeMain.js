$(function() {
    ...
    $('#allPlacesButton').on('click', function(event) {
        event.preventDefault(); // 기본 링크 동작 방지
        window.location.href = '/kkirikkiri/placeMain';
    });
    ...
    function filterPlaces(query) {
        ...
        $.ajax({
            url: '/kkirikkiri/place/filter',
            type: 'GET',
            data: {
                query: searchQuery,          // 검색어 또는 해시태그
                category: selectedActivity,  // 선택된 카테고리
                location: selectedLocation   // 지역 필터
            },
           success: function(data) {
				console.log(data);
	            var newContent = $(data).find('#category-container').html();
	            $('#category-container').html(newContent);
            
                if ($(data).find('#category-container').html().trim() === '') {
	...
	*/
    ...
    $('input[name="activity"]').on('change', function() {
        $('input[name="activity"]').not(this).prop('checked', false); // 다른 체크박스를 모두 해제
        filterPlaces();  // 필터링 함수 호출
    });
